...
namespace Tests
{
    public class Tests
    {
        ...
        public void CanEmbedParametersInQuery()
        {
            ...
            var query = parser.Parse(sb.ToString());

            Assert.That(query.WithParameters, Contains.Substring("SELECT this_.Id as y0_ FROM Person this_ WHERE Id = 1"));
            Assert.That(query.WithParameters, Contains.Substring("SELECT last_insert_rowid()"));
        }
        ...
        public void EmbedsParametersInReverseOrder()
        {
            ...
            Assert.That(query.WithParameters, Is.EqualTo("SELECT this_.Id as y0_ FROM Person this_ WHERE Id IN (1, 2)"));
        }
    }
}
